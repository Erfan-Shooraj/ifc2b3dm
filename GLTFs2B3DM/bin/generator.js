...
//var longitude = -1.31968;// -37.798546, 144.960446; //103.745380;//
//var latitude =0.698874;//  -37.798546; //1.336594;// 
// lat and lon are described in radians
var longitude = CesiumMath.toRadians(103.745253);   //144.960186
var latitude = CesiumMath.toRadians(1.336964);      //-37.798648
...
var buildingTemplate = {
    numberOfBuildings : 1, // default 10
    tileWidth : tileWidth,
    averageWidth : 8.0,
    averageHeight : 10.0,
    diffuseType : 'white',
...
function createRequestVolume() {
    var tilesetName = 'OurTilesetWithRequestVolume(3Walls)';
    var tilesetDirectory = path.join(outputDirectory, 'Samples', tilesetName);
    var tilesetPath = path.join(tilesetDirectory, 'tileset.json');
    //var buildingGlbPath = 'data/EDAS_5063817_v1_Q99_CET_URA_upgraded.glb';//'data/building.glb';
    //var buildingGlbPath = 'data/OfficeSample/20160414office_model_CV2_fordesign.glb';
    var buildingGlbPath = 'data/3walls/3walls.glb'; //only glb not gltf
    //var buildingTileName = 'EDAS_5063817_v1_Q99_CET_URA_upgraded.b3dm';
    var buildingTileName = '3walls.1.b3dm';
    var buildingTilePath = path.join(tilesetDirectory, buildingTileName);

    var buildingWidth = 6.02;
    var buildingDepth = 6.71;
    var buildingHeight = 4.17;
    var buildingGeometricError = 30.0; // Estimate based on diagonal
                                       // zero won't show; higher values will render from afar
    var buildingScale = 1.0;

	//var rotation = Matrix3.fromRotationZ(CesiumMath.PI/2); //rotate 90 degree
    var rotation = Matrix3.fromRotationZ(0);
    //var wgs84Matrix = wgs84Transform(longitude, latitude, -100.0); // lower the model 100 meter down
    var wgs84Matrix = wgs84Transform(longitude, latitude, 0.0);
    var scaleMatrix = Matrix4.fromUniformScale(buildingScale);
	
	var buildingMatrixWithRotation = Matrix4.multiplyByMatrix3(wgs84Matrix, rotation, new Matrix4()) ;
	
    var buildingMatrix = Matrix4.multiply(buildingMatrixWithRotation, scaleMatrix, new Matrix4());
	
	
    var buildingTransform = Matrix4.pack(buildingMatrix, new Array(16));
    var buildingBoxLocal = [
        -buildingWidth / 2.000, buildingDepth/2.00, buildingHeight/2.000, // center
        buildingWidth / 2.000, 0.0, 0.0,  // width
        0.0, buildingDepth / 2.000, 0.0,  // depth
        0.0, 0.0, buildingHeight / 2.000  // height
    ];
